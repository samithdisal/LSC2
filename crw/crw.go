...
func getContent(url string, basepath string) string {
	...
	fileName := fmt.Sprintf("%s%s_%s.txt", basepath, Marshal(author, true), Marshal(title, true))
...
func GetPub(url string, basepath string) {
	next := url
	for next != "" {
		next = getContent(next, basepath)
		//next = ""
		//time.Sleep(pauseDuration)
	}
}

func GetAuthor(url string, basepath string) {
	doc, err := goquery.NewDocument(url)
	if err != nil {
